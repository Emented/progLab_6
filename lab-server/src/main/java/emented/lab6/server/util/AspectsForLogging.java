...
public class AspectsForLogging {
    ...
    @After("execution(* emented.lab6.server.util.CommandManager.executeServerCommand(..))")
    public void executeServerCommandAdvise(JoinPoint joinPoint) {
        LOGGER.info("Executing server command: " + joinPoint.getArgs()[0]);
    }

    @After("execution(* emented.lab6.server.util.CommandManager.executeClientCommand(..))")
